...
int main(void) {
    ...
    assert(mybot_state_model_get_view(&model).app_state == MYBOT_STATE_WIFI_PROVISIONING);
    assert(!mybot_state_model_services_ready(&model));

    assert(mybot_state_model_begin_services(&model));
    assert(mybot_state_model_get_view(&model).app_state == MYBOT_STATE_STARTING_SERVICES);
    ...
    assert(mybot_state_model_get_view(&model).app_state == MYBOT_STATE_IN_CONVERSATION);

    assert(mybot_state_model_network_lost(&model));
    assert(mybot_state_model_get_view(&model).app_state == MYBOT_STATE_WIFI_DISCONNECTED);
    ...
    assert(mybot_state_model_get_view(&model).app_state == MYBOT_STATE_STOPPING);
    assert(!mybot_state_model_fail(&model));

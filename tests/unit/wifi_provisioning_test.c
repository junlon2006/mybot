...
static void on_state_changed(mybot_wifi_state_t state, void *user_data) {
...
int main(void) {
    ...
    assert(mybot_wifi_init(NULL, on_state_changed, &s_state_change_count) < 0);

    s_initial_event = MYBOT_WIFI_EVENT_STA_CONNECTED;
    assert(mybot_wifi_init("device-001", on_state_changed, &s_state_change_count) == 0);
    assert(s_init_count == 1);
    assert(mybot_wifi_get_state() == MYBOT_WIFI_STATE_CONNECTED);
    assert(aosl_atomic_read(&s_state_change_count) == 1);
    assert(aosl_atomic_read(&s_last_state) == MYBOT_WIFI_STATE_CONNECTED);
    assert(mybot_wifi_register(&fake_ops) < 0);

    s_fake.emit(MYBOT_WIFI_EVENT_STA_DISCONNECTED, s_fake.user_data);
    assert(mybot_wifi_get_state() == MYBOT_WIFI_STATE_DISCONNECTED);
    ...
    assert(mybot_wifi_get_state() == MYBOT_WIFI_STATE_IDLE);
    ...
    assert(mybot_wifi_get_state() == MYBOT_WIFI_STATE_PROVISIONING);

    for (int i = 0; i < 1000 && aosl_atomic_read(&s_state_change_count) == previous_state_changes;
         ++i) {
        aosl_hal_msleep(1);
    }
    assert(mybot_wifi_get_state() == MYBOT_WIFI_STATE_FAILED);
    assert(aosl_atomic_read(&s_state_change_count) == previous_state_changes + 1);
    assert(aosl_atomic_read(&s_last_state) == MYBOT_WIFI_STATE_FAILED);

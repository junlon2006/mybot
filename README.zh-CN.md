...
`mybot` 是面向设备端的跨平台语音机器人 SDK：负责 APSTA 配网、设备配对与认证、会话
状态机、双向实时音频（Agora RTSA）、按键/LCD 工作流，以及可选的本地唤醒词识别。
平台相关能力通过一组小型 `ops` 接口注入，SDK 核心不依赖 Linux ALSA、stdin 或文件
系统实现，可移植到 Linux 与各类 MCU 平台。
...
- **跨平台内核**：核心代码只依赖 AOSL 与平台 `ops` 契约，不直接触碰任何 OS 或外设
  API；同一份代码运行于 Linux 参考平台与 MCU 固件。
...
- **会话状态机**：`unprovisioned / pairing / awaiting_claim / runtime / in_conversation`
  五态驱动设备服务端交互。
- **双向实时音频**：基于 Agora RTSA，支持云端 AEC 与可选实时转写。
...
与平台 `ops` 契约之上，平台差异全部由各平台后端实现收敛。设备服务端和 Agora RTC 云
是运行时依赖的外部服务，均不在本仓库内。
...
- **外部服务**：设备服务端（配对 / 认领 / 会话调度，仅 HTTPS）与 Agora RTC（实时音视频）。
...
完整的服务端交互与设备侧状态定义见 [DEVICE_API.md](DEVICE_API.md)。
...
- [DEVICE_API.md](DEVICE_API.md) — 设备端服务端 API 对接规范
...
欢迎提交 issue、讨论与 PR。开始之前请阅读：

- [CONTRIBUTING.md](CONTRIBUTING.md) — 开发流程与提交规范
- [CODE_OF_CONDUCT.md](CODE_OF_CONDUCT.md) — 社区行为准则
- [SUPPORT.md](SUPPORT.md) — 获取支持的方式

#ifndef MYBOT_UTILS_HTTP_CLIENT_H_
#define MYBOT_UTILS_HTTP_CLIENT_H_
...
/** HTTP response returned by mybot_utils_http_get / mybot_utils_http_post. */
typedef struct {
...
} mybot_utils_http_response_t;

/**
 * @brief Simple blocking HTTP GET request.
 * @param url   URL in format "http://hostname[:port]/path"
 * @param resp  [out] response data (must call mybot_utils_http_response_free when done)
...
#endif /* MYBOT_UTILS_HTTP_CLIENT_H_ */

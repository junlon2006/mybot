...
void aosl_hal_atomic_init(void)
{
    // Stub - initialization no longer required with compiler intrinsics
}
...
{
    // Return old value: new - 1
    return __atomic_add_fetch((volatile intptr_t *)v, 1, __ATOMIC_SEQ_CST) - 1;
}
...
{
    intptr_t expected = old;
    __atomic_compare_exchange_n((volatile intptr_t *)v, &expected, new, false,
                                __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST);
    return expected; // Always returns the value of *v BEFORE the operation attempt
}

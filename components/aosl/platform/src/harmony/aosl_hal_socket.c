...
{
	/* HarmonyOS NEXT does not support SO_BINDTODEVICE.
	 * Return an explicit error so the upper layer knows this capability
	 * is unavailable on this platform. */
	(void)sockfd;
	(void)if_name;
	AOSL_LOG_ERR("SO_BINDTODEVICE is not supported on HarmonyOS NEXT");
...
int aosl_hal_sk_get_local_ip(aosl_sockaddr_t *addr)
{
	...
	/* HarmonyOS NEXT may not support AF_NETLINK for route queries.
	 * Instead of relying on the default route interface name,
	 * find the first non-loopback IPv4 address directly. */

...
static void *s_ctx;
static mybot_wifi_state_handler_t s_handler;
...
static void on_platform_event(mybot_wifi_event_t event, void *user_data) {
    ...
    case MYBOT_WIFI_EVENT_STA_CONNECTED:
        set_state(MYBOT_WIFI_STATE_CONNECTED);
        break;
    case MYBOT_WIFI_EVENT_STA_DISCONNECTED:
        set_state(MYBOT_WIFI_STATE_DISCONNECTED);
        break;
    case MYBOT_WIFI_EVENT_FAILED:
        set_state(MYBOT_WIFI_STATE_FAILED);
        break;
    }
}
...
int mybot_wifi_init(const char *device_id, mybot_wifi_state_handler_t handler, void *user_data) {
    ...
    if (s_ops->init(&s_ctx, device_id, on_platform_event, NULL) < 0) {
        ...
        s_handler = NULL;
        s_user_data = NULL;
        aosl_atomic_set(&s_state, MYBOT_WIFI_STATE_FAILED);
        return -1;
    }
...
mybot_wifi_state_t mybot_wifi_get_state(void) {
    return (mybot_wifi_state_t)aosl_atomic_read(&s_state);
}
...
void mybot_wifi_deinit(void) {
    ...
    s_handler = NULL;
    s_user_data = NULL;
    aosl_atomic_set(&s_state, MYBOT_WIFI_STATE_IDLE);
}

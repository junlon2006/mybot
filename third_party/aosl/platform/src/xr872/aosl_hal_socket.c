...
static int conv_domain_to_os(enum aosl_socket_domain domain)
{
	...
	case AOSL_AF_INET:
		return AF_INET;
	case AOSL_AF_INET6:
		return AF_INET; // Fallback to IPv4 since IPv6 is not enabled
		//return AF_INET6;
	default:
		return AF_UNSPEC;
	}
...
{
	//用于强制套接字只在指定网络接口（如 eth0/wlan0）收发数据，绕过系统路由表。
	// Temporarily commented out due to missing net/if.h
	// struct ifreq ifr;
...
int aosl_hal_sk_get_local_ip(aosl_sockaddr_t *addr)
{
	...
	// addr->sin_addr = netif->ip_addr.u_addr.ip4.addr;
	return 0;
}
...
int aosl_hal_gethostbyname(const char *hostname, aosl_sockaddr_t *addrs, int addr_count)
{
	...
	for (list = ent->h_addr_list; list != NULL && *list != NULL && count < addr_count; list++) {
		...
		addr.sin_addr = *(struct in_addr *)*list;
		addr.sin_family = AF_INET;

		conv_addr_to_aosl((struct sockaddr *)&addr, &addrs[count]);
		count++;
	}

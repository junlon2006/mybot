...
- LCD 渲染（包括会话中的声纹注册中/成功标记）、语义按键事件和 BK 按键输入；
- 基于 BK 环境存储的键值存储。
...
控制器将配网与 MyBot 作为两个独立的产品任务：进入 APSTA 配网前先停止 SDK，等待配网完成，
将成功提示音完整播报后，再在恢复的 STA 连接上启动 MyBot。对话期间，SDK 的
`MYBOT_LCD_INDICATOR_VP_REGISTERED` 会透传到两块 LCD；服务器确认注册前显示红色待注册标记，
确认后显示绿色成功标记。
